...
/**
 * Fetches win/loss records for all 30 NBA teams from the ESPN public API.
...
async function fetchStandings(seasonYear) {
  ...
  if (!response.ok) {
    throw new Error(`ESPN NBA API responded with ${response.status} for season ${seasonYear}`);
  }
